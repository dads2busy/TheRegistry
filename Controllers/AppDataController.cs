        ...
        public ActionResult Index(RegisterExternalLoginModel mReg, string Command, int id = 0)
        {
            ...
            if (Command == "VerificationsReport")
            {
                ...
                for (var i = 0; i < userIds.Count(); i++)
                {
                    ...
                    var firstOrDefault = _udb.Users.FirstOrDefault(x => x.Id == userInfo.UserId.ToString()); // .UserProfiles.FirstOrDefault(x => x.UserId == userInfo.UserId);
                    ...
                    var orDefault = responses.FirstOrDefault(x => x.QuestionText.ToUpper().Contains("FIRST"));
                    if (orDefault != null)
                        userInfo.FirstName = orDefault.ResponseItem;
                    var response = responses.FirstOrDefault(x => x.QuestionText.ToUpper().Contains("LAST"));
                    if (response != null)
